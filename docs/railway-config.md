...
| `preDeployCommand` | `/usr/local/bin/ratiba-migrate deploy` | **The most important line in this file.** Migrations run once, in a separate container, from the same image, *before* any new replica starts. If it fails, the release is aborted and the previous version keeps serving. This is what stops N replicas racing to migrate on startup. `deploy` is a single command because the distroless image has no shell to chain two with; it applies migrations and then refreshes the demo dataset. Seeding on every deploy is deliberate **for this assessment** — every environment holds fictitious data and a reviewer must be able to call the API without being handed identifiers first. It is idempotent and never touches appointments. A service holding real patient data would run `up` and seed nothing. |
